...
| A14 | S2 | Build stamp `.ver-tag` rendered at **2.54:1** — 9.5px `--dim` at `opacity:.65`. The contrast gate could not see it: it checks the TOKEN pair (`--dim` on `--bg` = 4.79:1) and never composites opacity, and the stamp's whole job is to be read in a screenshot | ✅ 3.1.4: opacity dropped (4.79:1 measured) + 9.5px → 11px. Photo Pointer hit the identical bug in its 1.14.2 — the cross-app rule is now: **dim text with a token, never with `opacity`**, or the gate goes blind. Map diagnostics (`.map-debug`, `.map-debug-copy`) 10.5px → 11px in the same pass (Noah reads them on-device). NEW sub-11px exemptions, same grounds as A10: `.score-q` (single "?" affordance in a 14px circle, chip carries its own full-size label) and `.sp-fi-btn ✓` (a third channel on top of colour + a 2px ring, so not the sole non-hue carrier) |
