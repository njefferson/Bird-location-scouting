...
export const CHANGELOG = [
  {
    ...
    changes: [
      ...
      'The map and the year planner dropped the colour scale entirely. Reading a smooth ramp of shades never worked — most dots crushed together and the best spots didn’t stand out. Now there are just two kinds of spot: each month’s hot spots — the natural top tier of that month’s ranking, usually the top dozen or so, found where the scores actually break — wear a vivid orange dot (a little bigger, drawn on top), and every other spot is a quiet plain dot. The planner works the same way: an orange cell means that hotspot is one of that month’s hot spots. One glance, no key to study, and the map follows the light and Dawn themes again.',
