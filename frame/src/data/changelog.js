...
export const CHANGELOG = [
  {
    ...
    changes: [
      ...
      'The numbers behind the dots are now visible. A ⓘ beside the map’s caption opens a statistics table — every site down the left, the twelve months across, and in each cell the actual count of reports (eBird checklists) the data holds for that site and month. Tap a month to sort by it. It makes the invisible scale plain: a month’s top spot can rest on 400 reports where another month’s rests on a dozen. Orange cells are each month’s strongest tier — the same spots as the orange pins — and the whole table is computed from the loaded data, so it updates itself with every data refresh.',
    ],
  },
  {
    version: 'v30',
