...
export const CHANGELOG = [
  {
    ...
    changes: [
      ...
      'The hotspot-map dots now read the way they should. A few standout locations used to dominate the colour maths, which crushed almost every other dot to near-white — the map looked blank and the key described a range you never saw. The colour now follows a curve that lifts the many modest spots into view, and the best spots wear a brighter, hotter orange so they stand out as bright highlights instead of dark shadows. The key is redrawn to match, and the year planner’s cells get the same treatment.',
