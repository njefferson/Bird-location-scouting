...
export const CHANGELOG = [
  {
    ...
    changes: [
      ...
      'The Ranking screen’s filter is redesigned. It’s always visible now (no more hunting behind “Filter by bird”): four category buttons — Type, Size, Nest, Behaviour — each open to their options, one at a time, with a plain-English reminder that a tap wants a group, a second excludes it, a third clears it. A small row of lights below always shows what you’re filtering by. And the bird icons on each hotspot card are now purely information — “here this month,” at a glance. Each icon carries its own state so there’s nothing extra to read: lit for a group that’s around, dimmed for one that only might be, and marked with a small ✕ for one you shouldn’t expect. They’re never a hidden control, so they can’t be confused with the filter.',
