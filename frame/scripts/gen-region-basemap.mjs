// =============================================================================
// gen-region-basemap.mjs — regenerate the basemap module for a NEW map area
// =============================================================================
// Parameterized sibling of gen-yellowstone-basemap.mjs: the same Natural Earth
// layers (coastline, rivers, lakes, roads, parks & protected lands) projected
// into each new area's OWN viewBox (from its *-shapes.js) and clipped to that
// area's county silhouette. Unlike the landlocked Yellowstone generator this
// one KEEPS the coastline layer — the Panama City cluster is on the Gulf; the
// inland Hahira cluster simply yields no coastline inside its silhouette. Emits
// the SAME export names as data/basemap.js so ui/basemap.js can pick the module
// by area. Output is committed and shipped offline.
//
//   node scripts/gen-region-basemap.mjs            (from frame/ — builds every area)
//   node scripts/gen-region-basemap.mjs panamacity (just one)
// =============================================================================
import { writeFileSync } from 'node:fs';

// Each area: its shapes module (for projection + silhouette) and its output.
const AREAS = {
  hahira: { shapes: '../src/data/hahira-shapes.js', prefix: 'HAHIRA', out: 'src/data/hahira-basemap.js' },
  panamacity: { shapes: '../src/data/panamacity-shapes.js', prefix: 'PCB', out: 'src/data/panamacity-basemap.js' },
};
...
// Fetch each source once, share across areas.
async function load(url) { const r = await fetch(url); if (!r.ok) throw new Error(`${r.status} ${url}`); return r.json(); }
const _cache = new Map();
async function loadCached(url) { if (!_cache.has(url)) _cache.set(url, await load(url)); return _cache.get(url); }
...
for (const key of areaKeys) {
  ...
  await buildArea(key, area);
}

async function buildArea(key, area) {
  const mod = await import(area.shapes);
  const P = mod[`${area.prefix}_PROJECTION`];
  const VB = mod[`${area.prefix}_VIEWBOX`];
  const SHAPES = mod[`${area.prefix}_SHAPES`];
  ...
  for (const code of Object.keys(SHAPES)) {
    ringsByCode[code] = [];
    for (const part of SHAPES[code].split('M').filter(Boolean)) {
      ...
      for (const f of gj.features) {
        ...
        for (const ring of toLines(f.geometry)) {
          for (const run of clipLine(ring)) {
            ...
            if (nm) { const [cx, cy] = polyCentroid(simp); const area2 = Math.abs(lineLen(simp)); const cur = byName.get(nm); if (!cur || area2 > cur.area) byName.set(nm, { area: area2, x: cx, y: cy }); }
          }
        }
      }
    }
  ...
  console.log(`[${key}] fetching + projecting Natural Earth landmarks…`);
  ...
  const banner = `// AUTO-GENERATED by scripts/gen-region-basemap.mjs — do not hand-edit.
// Orientation landmarks (Natural Earth, public domain) projected into the same
// ${W}×${H} viewBox as ${area.shapes.split('/').pop()}, clipped to that area's
// county silhouette. Same export shape as data/basemap.js (no curated point
// labels) so ui/basemap.js can pick the module by map area. Consumed via
// ui/basemap.js.
`;
  const arr = (name, xs) => `export const ${name} = [\n${xs.map((d) => `  '${d}',`).join('\n')}\n];\n`;
  const larr = (name, xs) => `export const ${name} = [\n${xs.map((l) => `  { x: ${l.x}, y: ${l.y}, t: ${JSON.stringify(l.t)} },`).join('\n')}\n];\n`;
  writeFileSync(area.out,
    banner + '\n' +
  ...
  console.log(`[${key}] wrote ${area.out} — ${total} paths, ${coastline.labels.length + rivers.labels.length + lakes.labels.length + roads.labels.length + parks.labels.length} labels`);
}
