...
- The hotspot map and the year planner are now proper heat-fields, and the value scale finally runs the right way. Before, on the light map the busiest spots were the darkest — so the best places read as shadows, and stripped of colour the scale ran backwards (bright = fewest birds). Now both sit on a dark ground and the spots glow from dim to bright: brighter always means more, so the best spots are the brightest, and it stays correct in greyscale or for colour-blind eyes. The colour also follows a curve so the many modest spots spread out instead of crushing to one end, and the key matches the dots exactly.
