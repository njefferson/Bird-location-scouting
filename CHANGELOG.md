...
- The hotspot-map dots now actually spread across the colour scale. A few standout locations used to dominate, which crushed almost every other dot to near-white — so the map looked blank and the white-to-orange key described a range you never saw. The colour now follows a curve that lifts the many modest spots into visible tans while the very best stay fullest orange, and the key is redrawn to match (and sits in a defined frame). The year planner’s cells get the same clearer spread.
