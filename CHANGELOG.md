...
- The map is easier to handle on a small screen. One finger now scrolls the page (the map used to swallow every touch, leaving nowhere to scroll from); pan the map with two fingers, pinch to zoom, and the caption says so. The dots are smaller at the opening county view so dense clusters read as dots instead of a solid mass, and hotspot names hold off until you’re zoomed in about twice as far as before, so they no longer flood the map the moment you start zooming.
